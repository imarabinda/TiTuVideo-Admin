...
function send_push($fcmtoken,$message,$plateform)
{
    if($plateform == 0)
    {
        ...
        if(is_array($fcmtoken)){
            ...
            $fields['registration_ids'] = array($fcmtoken);
        }
    ...
    else
    {
        ...
        if(is_array($fcmtoken)){
            ...
            $data['registration_ids'] = array($fcmtoken);
        }
...
function verify_request()
{
    ...
    function useridtodata($user_id){
        $ci = & get_instance();
        return $ci->db->select('full_name')->from('users')->where('user_id', $user_id)->get()->row_array();
    }
