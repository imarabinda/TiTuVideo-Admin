...
class Post extends REST_Controller {
        ...
        else
        {
            ...
            else
            {
                ...
                if(!empty($_FILES['post_video']['name'])){
                    ...
                    $config['allowed_types'] = 'mp4|aac|mkv|avi';
                    $config['file_name'] = 'video-'.rand(0,9999).time().rand(0,99999999999);
                  ...
                  if(!empty($_FILES['post_image']['name'])){
                    ...
                    $config['file_name'] = 'thumbnail-'.rand(0,9999).time().rand(0,99999999999);
                ...
                if(isset($_POST['can_save'])){
                    $data['can_save']=$this->input->post('can_save');
                }
                if(isset($_POST['can_comment'])){
                    $data['can_comment']=$this->input->post('can_comment');
                }
                if(isset($_POST['can_duet'])){
                    $data['can_duet']=$this->input->post('can_duet');
                }
                ...
                if($insert)
                {
                    ...
                    if($is_orignal_sound == 1)
                    {
                        ...
                        $config['file_name'] = 'sound-'.rand(0,9999).time().rand(0,99999999999);
                    ...
                    if(!empty($_FILES['sound_image']['name'])){
                        ...
                        $config['file_name'] = 'sound-thumbnail-'.rand(0,9999).time().rand(0,99999999999);
                    ...
                    $sound_data = array('sound'=>$post_sound,'sound_title'=>$sound_title,'duration'=>$duration,'added_by'=>$added_by,'singer'=>$singer,'sound_image'=>$sound_image);
    ...
    public function post_list_post()
    {
            ...
            else
            {
                ...
                foreach ($post_list as $value) {
                    ...
                    $ph[$x]['can_save'] = $value['can_save'];
                    $ph[$x]['can_comment'] = $value['can_comment'];
                    $ph[$x]['can_duet'] = $value['can_duet'];
    ...
    public function advertiser_list_post()
    {
        
        $created_date = date('Y-m-d H:i:s');
        
        $post_advertiser = $this->Post_model->advertiser_list();
        
        $ad  = 0;
        $adv = array();
        foreach ($post_advertiser as $advertiser_value) {
            
            $adv[$ad]['ad_img']               = $advertiser_value['ad_img'];
            $adv[$ad]['ad_brand_img']         = $advertiser_value['ad_brand_img'];
            $adv[$ad]['ad_brand_name']        = $advertiser_value['ad_brand_name'];
            $adv[$ad]['ad_brand_tag_line']    = $advertiser_value['ad_brand_tag_line'];
            $adv[$ad]['ad_brand_description'] = $advertiser_value['ad_brand_description'];
            $adv[$ad]['ad_price']             = $advertiser_value['ad_price'];
            $adv[$ad]['ad_brand_url']         = $advertiser_value['ad_brand_url'];
            $ad++;
        }
    ...
    public function sound_list_post()
    {
        ...
        else
        {
                ...
                foreach ($sound_category_list as $value) {
                    ...
                    $sound_list = $this->Post_model->limited_sound_list('sound', array('status'=>1,'sound_category_id'=>$value['sound_category_id']), $field = '*', 'sound_category_id');
    ...
    public function category_wise_sound_list_post()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $sound_list = $this->Post_model->limited_sound_list('sound', array('status'=>1,'sound_category_id'=>$sound_category_id), $field = '*', 'sound_category_id');
    ...
    public function hash_tag_search_video_post()
    {
        ...
        else
        {
            ...
            foreach ($hash_tag_search_video as $post_data_value) {
                ...
                $post_data[$p_count]['can_save'] = $post_data_value['can_save'];
                $post_data[$p_count]['can_comment'] = $post_data_value['can_comment'];
                $post_data[$p_count]['can_duet'] = $post_data_value['can_duet'];
    ...
    public function banners_post(){
        $banners = $this->Common->selectall_where_result('banners', array('status'=>1));
            $response = array(
                'status' => TRUE,
