...
class User extends REST_Controller {
    ...
    public function profile_category_list_get()
    {
        ...
        else
        {
            ...
            $data = $this->Common->selectall_where_result('profile_category', array('status'=>1));

            $response = array(
                'status' => TRUE,
                'message' => 'Profile category get successfully',
                'data' => $data,
            );
        }
    ...
    {
        return [
            "full_name","user_name","user_email","user_mobile_no","bio","fb_url","insta_url","youtube_url","profile_category"
            ];
    }
        ...
        {  
            ...
            else
            {
                ...
                if(!empty($_FILES['photo_id_image']['name'])){
                    ...
                    $config['file_name'] = rand(0,56789).'-verify-id-image-'.rand(0,9999).time();
                ...
                if(!empty($_FILES['photo_with_id_image']['name'])){
                    $config['upload_path'] = 'uploads/';
                    $config['allowed_types'] = 'jpg|png|jpeg';
                    $config['file_name'] = 'verify-id-with-'.rand(0,9999).'-image-'.rand(0,9999).time();
                ...
                $data = array(
                    ...
                    'name'=>$name,
                    'address'=>$address,
                    'photo_id_image'=>$photo_id_image ? $photo_id_image : 'no-photo-image-id.png',
                    'photo_with_id_image'=>$photo_with_id_image ? $photo_with_id_image : 'no-photo-with-id-image.png',
                    'created_date'=>$created_date
                );
    ...
    public function report_post()
    {
        ...
        {  
            ...
            else
            {
                ...
                $reported_by = $this->input->post('reported_by') ? $this->input->post('reported_by') : '';
    ...
    public function user_details_post()
    {
        ...
        else
        {
            ...
            $name = $this->db->where('profile_category_id',$user_details['profile_category'])->select('profile_category_name')->get('profile_category')->row_array();

            $user_details['profile_category_name']= $name['profile_category_name'];
    ...
    public function user_update_post()
    {
        ...
        else
        {
            ...
            if(!empty($_FILES['user_profile']['name'])){
                ...
                $config['file_name'] = 'pr-'.rand(0,9999).'-ofile-'.rand(0,9999).time();
            ...
            $name = $this->db->where('profile_category_id',$userdata['profile_category'])->select('profile_category_name')->get('profile_category')->row_array();

            $userdata['profile_category_name']= $name['profile_category_name'];
